...
function getIconPath() {
    let suffix = iconPrefixMap[process_1.default.platform];
    suffix = suffix ? suffix : '.png';
    return path.join(electron_1.app.getAppPath(), 'icon' + suffix);
}
const createWindow = () => {
    ...
    mainWindow.loadURL('http://localhost:3000/main_window').then(() => { });
    electron_1.session.defaultSession.clearCache();
    mainWindow.on('close', () => {
        electron_1.session.defaultSession.clearStorageData().then(r => console.log(r));
    });
