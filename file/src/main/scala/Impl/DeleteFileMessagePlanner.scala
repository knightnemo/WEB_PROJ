...
case class DeleteFileMessagePlanner(fileId: Int, override val planContext: PlanContext) extends Planner[String]:
  ...
  override def plan(using PlanContext): IO[String] = {
    val sql = s"DELETE FROM ${schemaName}.files WHERE file_id = ?"
    val params = List(ParameterList(List(SqlParameter("Int", fileId.toString))))
